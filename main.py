...
def load_data():
    ...
    prev_X_tr = np.load('data/octave2_prev_x.npy')
    y_tr    = np.load('data/chord_condition.npy')
...
def main():
    ...
    if is_sample == 1:
        ...
        nz = 100
        n_bars = 7
        X_te = np.load('data/octave2_x.npy')
        prev_X_te = np.load('data/octave2_prev_x.npy')
        prev_X_te = prev_X_te[:,:,check_range_st:check_range_ed,:]
        y_te    = np.load('data/chord_condition.npy')
